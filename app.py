...
with tab1:
    ...
    if uploaded_file is not None:
        ...
        if all_extracted_rows:
            ...
            def render_selection_info(select_event, target_df):
                """Safely renders information for clicked dataframe cells."""
                if not select_event or not isinstance(select_event, dict):
                    return
                ...
                cell_info = cells[0]
                sel_row_idx = cell_info[0] if isinstance(cell_info, (tuple, list)) else cell_info.get("row")
                sel_col_name = cell_info[1] if isinstance(cell_info, (tuple, list)) else cell_info.get("column")
                
                if sel_row_idx is not None and 0 <= sel_row_idx < len(target_df):
                    row_data = target_df.iloc[sel_row_idx]
            ...
            # --- Table 1: Diameter Zone Verification Table ---
            st.markdown("---")
            ...
            render_selection_info(p3_select, p3_df)

            # --- Table 2: Cross-Machine Comparison Table ---
            st.markdown("---")
            ...
            render_selection_info(p4_select, p4_df)
            ...
            render_selection_info(p5_select, p5_df)
